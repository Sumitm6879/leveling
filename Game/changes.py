...
class Economy(commands.Cog):
    ...
    async def fight(self, ctx, member:discord.Member):
        search = player_search(ctx.author.id)
        second_person = player_search(member.id)
        if search is None or second_person is None:
            if search is None:
                tada = new_to_this(ctx)
                return await ctx.send(tada)
            elif member is None:
                tada = f"{ctx.author.mention} you can't do this!\n**{member.name}** has never played!"
                return await ctx.send(tada)
